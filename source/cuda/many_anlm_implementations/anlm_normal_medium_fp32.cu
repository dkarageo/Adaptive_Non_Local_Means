 ...
 * -------------------------IMPORTANT NOTICE----------------------------------
 * --------------------------TARGET=GTX 480-----------------------------------
 * THIS IS THE EXACT SAME IMPLEMENTATION CONTAINED IN anlm_normal.cu WITH
 * DIFFERENT LAUNCH BOUNDS. IT TARGETS FP32 IMAGES OF SIZE 256*256 OR GREATER.
 * IT HAS BEEN FOUND TO PROVIDE 1-5% IMPROVEMENT ON GTX 480 BY LIMITING
 * COMPILER'S DEFAULT REGISTER USAGE AND THUS INCREASING OCCUPANCY.
 * ---------------------------------------------------------------------------
 *
...
template <class T>
void
adaptiveNonLocalMeansNormalData_mediumFP32(
    T *src, T *dst, int *ids, T *filterSigma, int imgH, int imgW,
    int patchH, int patchW, T patchSigma, int regions)
{
    using std::vector;
...
template
void
adaptiveNonLocalMeansNormalData_mediumFP32<float>(float *, float *, int *, float *,
