...
template<class T>
__global__ void
cudaAnlmKernel(DMat<T> dRSrc, DMat<T> dRPatches, DMat<T> dRPatchesT, DMat<T> dDst,
...
template <class T>
__global__ void
cudaTranspose(DMat<T> src, DMat<T> dst);
                                ...
                                T patchSigma, int regions)
{
    ...
    vector<DMat<T>>       dRegionsPatchesT(regions);
    ...
    for (int i = 0; i < regions; ++i) {
        ...
        dRegionsPatchesT[i]     = DMat<T>(patchH*patchW, regionSizes[i], false);
    ...
    // Create a transposed copy of all dRegionsPatches.
    for (int i = 0; i < regions; ++i) {
        cudaTranspose<<<rGridDims[i], rBlockDims[i], 0, streams[i]>>>(
            dRegionsPatches[i], dRegionsPatchesT[i]
        );
    }
    ...
    for (int i = 0; i < regions; ++i) {
        cudaAnlmKernel<<<rGridDims[i], rBlockDims[i], 0, streams[i]>>>(
                dRegionsSrc[i], dRegionsPatches[i], dRegionsPatchesT[i], dDst,
                dOriginalPos[i], dRegionsFiltersSigma[i], patchH, patchW
        );
               ...
               int patchH, int patchW)
{
    ...
    const int targetX  = threadIdx.x % patchSize;  // Pixel in patch that current thread will load.
    const int targetY  = threadIdx.x / patchSize;  // Patch whose pixel current thread will load.

    // Calculate weights excluding the remainder patches (of chunks division).
    for (int cI = 0; cI < chunks; ++cI) {
        __syncthreads();
        sPatches[threadIdx.x] = dRPatchesT(cI*patchesC+targetY, targetX);
        __syncthreads();
    ...
    if (threadIdx.x < rem*patchSize) {
        sPatches[threadIdx.x] = dRPatchesT(chunks*patchesC+targetY, targetX);
    }
    __syncthreads();
...
/**
 * Kernel that copies transposed src to to dst.
 */
template <class T>
__global__ void
cudaTranspose(DMat<T> src, DMat<T> dst)
{
    ...
    for (int i = 0; i < srcHeight; ++i) dst(x, i) = src(i, x);
}
