               ...
               int patchH, int patchW)
{
    ...
    const int maxP     = chunks * patchesC;

    __shared__ T sSrc[BLOCK_SIZE];
    const int ppl = (BLOCK_SIZE / patchesC) * patchesC;  // Patches to be served by each sSrc load.
    int curSrcBlock = 0;

    // Calculate weights excluding the remainder patches (of chunks division).
    for (int cI = 0; cI < maxP; cI += patchesC) {
        __syncthreads();
        sPatches[threadIdx.x] = dRPatches(targetY, cI+targetX);
        if (cI%ppl == 0) {
            sSrc[threadIdx.x] = dRSrc(0, cI+threadIdx.x);
            curSrcBlock = 0;
        }
        __syncthreads();
        ...
        for (int i = 0; i < patchesC; ++i) {
            sI += patchSize;
            if (cI+i == x) continue;  // Do not count the weight against itself.

            T weight = 0;
            for (int j = 0; j < patchSize; ++j) {
                T d = dRPatches(j, x) - sPatches[sI+j];
                weight += d*d;
            }
            weight = __anlm_exp<T>(-weight/fSigma);
            if (weight > maxWeight) maxWeight = weight;  // will use max weight for itself

            nom   += sSrc[curSrcBlock+i] * weight;
            denom += weight;
        }
        
        curSrcBlock += patchesC;
    }
