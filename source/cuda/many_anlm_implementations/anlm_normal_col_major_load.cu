               ...
               int patchH, int patchW)
{
    ...
    const int targetY  = threadIdx.x / patchesC;  // Pixel in patch that current thread will load.
    const int targetX  = threadIdx.x % patchesC;  // Patch whose pixel current thread will load.

    // Calculate weights excluding the remainder patches (of chunks division).
    for (int cI = 0; cI < chunks; ++cI) {
        __syncthreads();
        if (threadIdx.x < patchesC*patchSize) {
            sPatches[patchSize*targetX+targetY] = dRPatches(targetY, cI*patchesC+targetX);
        }
        __syncthreads();
    ...
    __syncthreads();
    if (threadIdx.x < patchesC*patchSize) {
        sPatches[patchSize*targetX+targetY] = dRPatches(targetY, chunks*patchesC+targetX);
    }
    __syncthreads();
