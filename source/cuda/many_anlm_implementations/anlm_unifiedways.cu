 ...
 *
 * An effor to remove one branch into kernel, by writing the src of next
 * pixel location (forward or backwards) to a pointer.
 *
 * Version: 0.1
 */
               ...
               int patchH, int patchW)
{
    ...
    DMat<DMatPos> *nextCellSrc = &dNext;
    bool forward = true;
    ...
    while (true) {
        ...
        curCell = nextCellSrc->at(curCell);
