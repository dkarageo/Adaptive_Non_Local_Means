               ...
               int patchH, int patchW)
{
    ...
    int sSrcPos = 0;
    sSrc[threadIdx.x] = dRSrc(0, threadIdx.x);
    __syncthreads();

    // Calculate weights excluding the remainder patches (of chunks division).
    for (int cI = 0; cI < chunks; sSrcPos += patchesC, ++cI) {
        __syncthreads();
        sPatches[threadIdx.x] = dRPatches(targetY, cI*patchesC+targetX);
        if (sSrcPos == ppl) {
            sSrc[threadIdx.x] = dRSrc(0, cI*patchesC+threadIdx.x);
            sSrcPos = 0;
        }
        __syncthreads();
