...
int main(int argc, char *argv[])
{
    ...
    // vector<float> noisyImgFloat(noisyImg.begin(), noisyImg.end());
    // vector<float> filteredImgFloat(noisyImgFloat.size());
    //
    // vector<floa
                        ...
                        int regionsNum)
{
    ...
    for (int i = 0; i < stds.size(); ++i) stds[i] = sqrt(stds[i]/(regionSize[i]-1));
